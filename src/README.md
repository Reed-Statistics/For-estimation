This subfolder contains source code for this project. `data_tidy.R` contains the code for transforming and tidying the raw data from the U.S. Forest Service. The variable codebook (`variable_codebook.Rmd`) describes the relevant predictor and response variables. `create_strata.R` contains the code to wrangle data into strata, and `new_strata.R` contains the procedure to collapse strata with insufficient county-level observations. `s_mean.R` and `s_var.R` contain functions to calculate post-stratification sample mean and variance, respectively. 
